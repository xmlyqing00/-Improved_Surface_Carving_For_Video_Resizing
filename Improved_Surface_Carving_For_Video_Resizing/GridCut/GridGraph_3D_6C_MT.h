...
public:
  ...
  type_ncap* cap_eel;
  type_ncap* cap_eeg;
  inline void init_cap(int N) {
	  cap_source = new int[N];
	  cap_sink = new int[N];
	  cap_lee = new int[N];        // [-1, 0, 0]
	  cap_gee = new int[N];        // [+1, 0, 0]
	  cap_ele = new int[N];        // [ 0,-1, 0]
	  cap_ege = new int[N];        // [ 0,+1, 0]
	  cap_eel = new int[N];        // [ 0, 0,-1]
	  cap_eeg = new int[N];        // [ 0, 0,+1]
...
int GridGraph_3D_6C_MT<type_tcap,type_ncap,type_flow>::find_origin(int v,const int TIME)
{
  ...
  while(1)
  {
    if (timestamp[v]==TIME)    { goto find_origin_L1; }
    if (PARENT(v) == NONE)     { return INF_D; }
	if ( PARENT( v ) == TERMINAL ) { goto find_origin_L2; }
